...
class unreconcile_statement_entries(osv.osv_memory):
    ...
    def _get_defaults(self, cr, uid, context, result='None'):
        #logging.getLogger(self._name).warn('_get_defaults, context=%s', context)
        ...
        if voucher_ids:
            ...
            note += voucher_note
            #logging.getLogger(self._name).warn('_get_defaults, voucher_note=%s', voucher_note)

        if not (reconcile_ids or voucher_ids):
            raise osv.except_osv(_('Information !'), _('There are no reconciled Entries in this Bank Statement.'))
        else:
    ...
    def view_invoices(self, cr, uid, ids, context):
        inv_ids = self.read(cr, uid, ids, ['inv_ids'], context=context)[0]['inv_ids']
        #logger.notifyChannel('addons.'+self._name, netsvc.LOG_WARNING, 'inv_ids = %s' % inv_ids)
        return {
