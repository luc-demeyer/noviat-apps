...
class account_bank_statement_line(orm.Model):
    ...
    def create_move(self, cr, uid, st_line_id, company_currency_id, st_line_number, context=None):
        ...
        - the secondary currency fields are always completed
        ...
        if st_line.move_ids:
            ...
            if st.journal_id.default_credit_account_id != st.journal_id.default_debit_account_id:
                raise orm.except_orm(_("Unsupported Function !"),
            ...
            if st.currency.id == company_currency_id:
                if not st_line.account_id.currency_id:
                    val['currency_id'] = company_currency_id
                    val['amount_currency'] = -amount
                else:
                    if st_line.account_id.currency_id.id == company_currency_id:
                        val['currency_id'] = company_currency_id
                        val['amount_currency'] = -amount
                    else:
                        val['currency_id'] = st_line.account_id.currency_id.id
                        amount_cur = res_currency_obj.compute(cr, uid, company_currency_id,
                                st_line.account_id.currency_id.id, st_line.amount, context=context_create_move)
                        val['amount_currency'] = -amount_cur

            # statement in foreign currency
            else:
                if not st_line.account_id.currency_id:
                    val['currency_id'] = st.currency.id
                    val['amount_currency'] = -st_line.amount
                else:
                    if st_line.account_id.currency_id.id == st.currency.id:
                        val['currency_id'] = st.currency.id
                        val['amount_currency'] = -st_line.amount
                    else:
                        val['currency_id'] = st_line.account_id.currency_id.id
            ...
            if context.get('tax_code_id'):
                val['tax_code_id'] = context['tax_code_id']
                val['tax_amount'] = val['debit'] or val['credit']

            move_line_id = account_move_line_obj.create(cr, uid, val, context=context)

            # create counterpart_move on journal account           
            ...
            if st_line.account_id.type in ['receivable', 'payable']:
                ...
                if not st_line.voucher_id and not st_line.move_ids:
                    ...
                    if len(entries) > 1:
                        ...
                        ctx_voucher_create = dict(context, active_ids=[st_line.id], active_id=st_line.id, 
                            entries=[x['id'] for x in entries], partner_id=partner_id, company_id=company.id)
                        act_voucher_create = {
                    ...
                    if line_cr_ids or line_dr_ids:
                        voucher_id = voucher_obj.create(cr, uid, voucher_vals, context=dict(context, active_ids=[st_line.id], active_id=st_line.id, journal_id=journal.id))
...
class account_bank_statement(orm.Model):
    ...
    def button_cancel(self, cr, uid, ids, context=None):
        ...
        Since the method is replaced (for statements of type 'bank', also the logic in the inherited 'account_bank_statement_extensions' module is included.
        """
        done = []
        for st in self.browse(cr, uid, ids, context=context):
            if st.state=='draft':
            ...
            if st.journal_id.type == 'bank':
                ...
                # also add inherited logic of account_bank_statement_extensions 
                if st.line_ids and self.pool.get('ir.module.module').search(cr, uid, [('name', '=', 'account_bank_statement_extensions'), ('state', '=', 'installed')]):
                    cr.execute("UPDATE account_bank_statement_line  \
