...
class account_voucher(orm.Model):
    ...
    _columns = {
        'active': fields.boolean('Active', 
            help="By default, reconciliation vouchers made on draft bank statements are set as inactive, which allow to hide the customer/supplier payment while the bank statement line isn't reconciled."),
    ...
    def _reopen(self, context):
        act_voucher = pickle.loads(str(context['wizard_action']))
        act_voucher['context'] = context
        return act_voucher

    def absv_proforma_voucher(self, cr, uid, ids, context=None):
        wf_service = netsvc.LocalService("workflow")
        for vid in ids:
            wf_service.trg_validate(uid, 'account.voucher', vid, 'proforma_voucher', cr)
            self.write(cr, uid, [vid], {'active': True}, context=context)
        return self._reopen(context)

    def absv_cancel_voucher(self, cr, uid, ids, context=None):
        wf_service = netsvc.LocalService("workflow")
        for vid in ids:
            wf_service.trg_validate(uid, 'account.voucher', vid, 'cancel_voucher', cr)
        return self._reopen(context)

    def absv_unreconcile_voucher(self, cr, uid, ids, context=None):
        super(account_voucher, self).cancel_voucher(cr, uid, ids, context=context)
        return self._reopen(context)

    def absv_button_dummy(self, cr, uid, ids, context=None):
        self.write(cr, uid, ids, {}, context=context)
        return self._reopen(context)

    def absv_button_close(self, cr, uid, ids, context=None):
        self.write(cr, uid, ids, {}, context=context)
        return {'type': 'ir.actions.act_window_close'}

    def absv_action_cancel_draft(self, cr, uid, ids, context=None):
        super(account_voucher, self).action_cancel_draft(cr, uid, ids, context=context)
        return self._reopen(context)
