    ...
    def coda_parsing(self, cr, uid, ids, context=None, batch=False, codafile=None, codafilename=None):
        ...
        for line in recordlist:
            ...
            if not line:
                ...
                if line[1] == '1':
                    ...
                    coda_statement['coda_statement_lines'][st_line_seq]['counterparty_bic'] = line[98:109].replace(' ','')
                    ...
                    if coda_version == '1':
                        counterparty_number = line[10:22].replace(' ','')
                        ...
                        if line[22] == ' ':
                            ...
                            counterparty_number = line[10:44].replace(' ','')
                    ...
                    if st_line['type'] == 'general':                    
                        ...
                        if payment_reference and find_payment and st_line['amount'] < 0:
                            payline_ids = payment_line_obj.search(cr, uid, [('name', '=', payment_reference)])
                            if payline_ids:
                        ...
                        if match and st_line['account_id'] != transfer_acc and counterparty_number and update_partner:
                            ...
                            if len(partner_bank_ids) > 1:
                                ...
                                logger.notifyChannel('addons.'+self._name, netsvc.LOG_WARNING,
                                    'Duplicate Bank Accounts for partner_id %s have been removed, ids = %s' % (st_line['partner_id'],partner_bank_ids[:-1]))
        ...
        for statement in coda_statements:
            ...
            try:
                ...
                # create only bank statememt for CODA's of type 'normal' containing transaction lines
                ...
                for x in lines:
                    ...
                    if not line['type'] == 'communication':
                        if line['trans_family'] in st_name_families:
                            line['name'] = get_st_name(line, context)                       
                        if line['type'] == 'information':
                            if line['struct_comm_type'] in parse_comms_info:
                                line['name'], line['communication'] = parse_comm_info(line, comm_type_table, context)
                            elif line['struct_comm_type'] in parse_comms_move:
                                line['name'], line['communication'] = parse_comm_move(line, comm_type_table, context)                                
                        elif line['struct_comm_type'] in parse_comms_move:
                                line['name'], line['communication'] = parse_comm_move(line, comm_type_table, context)
...
def get_st_name(line, context):
    #logging.getLogger('get_st_name - entry').warn('ref = %s, family = %s, code = %s, categ = %s', line['ref'], line['trans_family'], line['trans_code'], line['trans_category'])
    st_name = line['name']

    if line['trans_family'] == '35' and line['trans_code'] in ['01', '37']:
        st_name = ', '.join([_('Closing'), line['trans_code_desc'], line['trans_category_desc']])

    if line['trans_family'] in ['13', '41']:
        st_name = ', '.join([line['trans_family_desc'], line['trans_code_desc'], line['trans_category_desc']])

    if line['trans_family'] in ['80']:
        st_name = ', '.join([line['trans_code_desc'], line['trans_category_desc']])

    #logging.getLogger('get_st_name - exit').warn('st_name = %s', st_name)
    return st_name

def parse_comm_move(line, comm_type_table, context):
    #logging.getLogger('parse_comm_move - entry').warn('ref = %s, family = %s, code = %s, categ = %s, comm_type = %s, name = %s', line['ref'], line['trans_family'], line['trans_code'], line['trans_category'], line['struct_comm_type'], line['name'])
    comm_type = line['struct_comm_type']
    comm = st_comm = line['communication']
    st_name = line['name']

    if comm_type in ['115', '121', '122', '126', '127']:
        logging.getLogger('Unsupported Communication Type').warn('The parsing of Structured Commmunication Type %s has not yet been implemented. ' \
            'Please contact Noviat (info@noviat.be) for more information about the development roadmap', comm_type)

    if comm_type == '100':
        st_name = _('Payment with ISO 11649 structured format communication')
        st_comm = '\n' + indent + _('Payment with a structured format communication applying the ISO standard 11649') + ':'
        st_comm += indent + _('Structured creditor reference to remittance information')
        st_comm += indent + comm

    if comm_type in ['101', '102']:
        st_name = st_comm = '+++' + comm[0:3] + '/' + comm[3:7] + '/' + comm[7:12] + '+++'

    if comm_type == '103':
        st_name = ', '.join([line['trans_family_desc'], _('Number')])
        st_comm = comm

    if comm_type == '105':
        st_name = filter(lambda x: comm_type == x['code'], comm_type_table)[0]['description']
        ...
        st_comm = '\n' + indent + st_name + indent + _('Gross amount in the currency of the account') + ': %.2f' % amount_1
        st_comm += indent + _('Gross amount in the original currency') + ': %.2f' % amount_2
        st_comm += indent + _('Rate') + ': %.4f' % rate
        st_comm += indent + _('Currency') + ': %s' % currency
        st_comm += indent + _('Structured format communication') + ': %s' % struct_format_comm
        st_comm += indent + _('Country code of the principal') + ': %s' % country_code
        st_comm += indent + _('Equivalent in EUR') + ': %.2f' % amount_3

    if comm_type == '106':
        if not st_name and line['trans_family'] not in st_name_families:
            st_name = _('VAT, withholding tax on income, commission, etc.')
        interest = comm[30:42].strip('0')
        st_comm = '\n' + indent + st_name + indent + _('Equivalent in the currency of the account') + ': %.2f' % list2float(comm[0:15])
        st_comm += indent + _('Amount on which % is calculated') + ': %.2f' % list2float(comm[15:30])
        st_comm += indent + _('Percent') + ': %.4f' % number2float(comm[30:42], 8)
        st_comm += indent + (comm[42] == 1 and _('Minimum applicable') or _('Minimum not applicable'))
        st_comm += indent + _('Equivalent in EUR') + ': %.2f' % list2float(comm[43:58])
    ...
    if comm_type == '107':
        ...
        st_comm = '\n' + indent + st_name + indent + _('Direct Debit Number') + ': %s' % direct_debit_number
        st_comm += indent + _('Central (Pivot) Date') + ': %s' % pivot_date
        st_comm += indent + _('Communication Zone') + ': %s' % comm_zone
        st_comm += indent + _('Paid or reason for refusal') + ': %s' % paid_refusal
        st_comm += indent + _("Creditor's Number") + ': %s' % creditor_number

    if comm_type == '108':
        st_name = _('Closing, period from %s to %s') % (str2date(comm[42:48]), str2date(comm[48:54]))
        interest = comm[30:42].strip('0')
        st_comm = '\n' + indent + st_name + indent + _('Equivalent in the currency of the account') + ': %.2f' % list2float(comm[0:15])
        if interest:
            st_comm += indent + _('Interest rates, calculation basis') + ': %.2f' % list2float(comm[15:30]) + \
    ...
    if comm_type =='111':
        ...
        trans_types = {
            ...
            '8': _('Teledata'),
            '9': _('Fuel')}
        st_name = _('POS credit - globalisation')
        ...
        st_comm = '\n' + indent + st_name + indent + _('Card Scheme') + ': %s' % card_scheme
        st_comm += indent + _('POS Number') + ': %s' % pos_number
        st_comm += indent + _('Period Number') + ': %s' % period_number
        st_comm += indent + _('First Transaction Sequence Number') + ': %s' % first_sequence_number
        st_comm += indent + _('Date of first transaction') + ': %s' % trans_first_date
        st_comm += indent + _('Last Transaction Sequence Number') + ': %s' % last_sequence_number
        st_comm += indent + _('Date of last transaction') + ': %s' % trans_last_date
        st_comm += indent + _('Transaction Type') + ': %s' % trans_type
        st_comm += indent + _('Terminal Identification') + ': %s' % terminal_name + ', ' + terminal_city
    ...
    if comm_type == '113':
        ...
        st_comm = '\n' + indent + st_name + indent + _('Card Number') + ': %s' % card_number
        st_comm += indent + _('Card Scheme') + ': %s' % card_scheme
        if terminal_number:
            st_comm += indent + _('Terminal Number') + ': %s' % terminal_number
        st_comm += indent + _('Transaction Sequence Number') + ': %s' % sequence_number
        st_comm += indent + _('Time') + ': %s' % trans_date + ' ' + trans_hour
        st_comm += indent + _('Transaction Type') + ': %s' % trans_type
        st_comm += indent + _('Terminal Identification') + ': %s' % terminal_name + ', ' + terminal_city
        if orig_amount:
            st_comm += indent + _('Original Amount') + ': %.2f' % orig_amount
            st_comm += indent + _('Rate') + ': %.4f' % rate
            st_comm += indent + _('Currency') + ': %s' % currency
        if volume:
            st_comm += indent + _('Volume') + ': %.2f' % volume
        if product_code:
            st_comm += indent + _('Product Code') + ': %s' % product_code
        if unit_price:
            st_comm += indent + _('Unit Price') + ': %.2f' % unit_price
    ...
    if comm_type == '114':
        ...
        trans_types = {
            ...
            '8': _('Teledata'),
            '9': _('Fuel')}
        st_name = _('POS credit - individual transaction')
        card_scheme = card_schemes.get(comm[0], '')
        pos_number = comm[1:7].strip()
        period_number = comm[7:10].strip()
        ...
        st_comm += indent + _('Transaction Type') + ': %s' % trans_type
        st_comm += indent + _('Terminal Identification') + ': %s' % terminal_name + ', ' + terminal_city
        st_comm += indent + _('Transaction Reference') + ': %s' % trans_reference
    ...
    if comm_type == '123':
        ...
        st_comm = '\n' + indent + st_name + indent + _('Starting Date') + ': %s' % starting_date
        st_comm += indent + _('Maturity Date') + ': %s' % maturity_date
        st_comm += indent + _('Basic Amount') + ': %.2f' % basic_amount
        st_comm += indent + _('Percentage') + ': %.4f' % percent
        st_comm += indent + _('Term in days') + ': %s' % term
        st_comm += indent + (minimum and _('Minimum applicable') or _('Minimum not applicable'))
        st_comm += indent + _('Guarantee Number') + ': %s' % guarantee_number
    ...
    if comm_type == '124':
        card_issuers = {
            ...
            '4': 'Diners Club',
            '9': _('Other')}
        st_name = _('Settlement credit cards')
        ...
        st_comm = '\n' + indent + st_name + indent + _('Card Number') + ': %s' % card_number
        st_comm += indent + _('Issuing Institution') + ': %s' % card_issuer
        st_comm += indent + _('Invoice Number') + ': %s' % invoice_number
        st_comm += indent + _('Identification Number') + ': %s' % identification_number
        st_comm += indent + _('Date') + ': %s' % date
                
    if comm_type == '125':
        if line['trans_family'] not in st_name_families:
            st_name = _('Credit')
        ...
        st_comm = '\n' + indent + st_name + indent + _('Credit Account Number') + ': %s' % credit_account
        st_comm += indent + _('Old Balance') + ': %.2f' % old_balance
        st_comm += indent + _('New Balance') + ': %.2f' % new_balance
        st_comm += indent + _('Amount') + ': %.2f' % amount
        st_comm += indent + _('Currency') + ': %s' % currency
        st_comm += indent + _('Starting Date') + ': %s' % start_date
        st_comm += indent + _('End Date') + ': %s' % end_date
        st_comm += indent + _('Nominal Interest Rate or Rate of Charge') + ': %.4f' % rate
        st_comm += indent + _('Transaction Reference') + ': %s' % trans_reference
        
    return st_name, st_comm

def parse_comm_info(line, comm_type_table, context):
    comm_type = line['struct_comm_type']
    comm = st_comm = line['communication']
    st_name = line['name']
    ...
    if comm_type == '001':
        st_name = filter(lambda x: comm_type == x['code'], comm_type_table)[0]['description']
        st_comm = '\n' + indent + st_name + indent + _('Name') + ': %s' % comm[0:70].strip()
        st_comm += indent + _('Street') + ': %s' % comm[70:105].strip()
        st_comm += indent + _('Locality') + ': %s' % comm[105:140].strip()
        st_comm += indent + _('Identification Code') + ': %s' % comm[140:175].strip()
    ...
    if comm_type == '006':
        ...
        st_name = filter(lambda x: comm_type == x['code'], comm_type_table)[0]['description']
        st_comm = '\n' + indent + st_name + indent + _('Description of the detail') + ': %s' % comm[0:30].strip()
        st_comm += indent + _('Amount') + ': %s' % amount
        st_comm += indent + _('Category') + ': %s' % comm[49:52].strip()
