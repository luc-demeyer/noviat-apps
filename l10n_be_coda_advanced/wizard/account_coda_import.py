...
class account_coda_import(orm.TransientModel):
    ...
    _columns = {
        ...
        'coda_fname_dummy': fields.related('coda_fname', type='char', string='CODA Filename', readonly=True),
    ...
    def _check_account_payment(self, cr, uid, context=None):
        res = self.pool.get('ir.module.module').search(
            cr, uid, [
                ('name', '=', 'account_payment'),
                ('state','=','installed')])
        return res and True or False

    def onchange_fname(self, cr, uid, ids, coda_fname):
        return {'value': {'coda_fname_dummy': coda_fname}}

    def coda_parsing(self, cr, uid, ids, context=None, batch=False,
        ...
        self.digits = self.pool.get('decimal.precision').precision_get(
            cr, uid, 'Account')
        ...
        for line in recordlist:
            ...
            if not line:
                ...
                cba_filter = lambda x: (coda_statement['acc_number'] in _get_acc_numbers(x['acc_number'])) \
                    and (coda_statement['currency'] == x['currency_name']) \
                    and (coda_statement['description'] in [x['description1'] or '', x['description2'] or ''])
                ...
                if not skip:
                    ...
                    if coda_bank['coda_st_naming']:
                        ...
                        if context.get('coda_id'):
                            search_obj = coda_bank['state'] == 'normal' and \
                    ...
                    if not skip:
                        coda_statement, coda_import_note = \
                            self._coda_statement_init(
                                cr, uid, coda_statement,
                                coda_import_note, context=context)
                ...
                if not period_id:
                    if coda_statement['new_balance_date']:
                        period_id = period_obj.search(cr , uid, [
                            ...
                            ('special', '=', False), ('company_id', '=', coda_statement['company_id'])
                            ])
                    else:
                        period_id = period_obj.search(cr , uid, [
            ...
            try:
                ...
                if statement['type'] == 'info':
                    ...
                    if not discard:
                        ...
                        st_hook_error, st_vals = self._statement_hook(
                            cr, uid, statement['coda_bank_params'],
                            st_vals, context=context)
                        if st_hook_error:
                            nb_err += 1 
                            err_string += st_hook_error
                            break
                        bk_st_id = bank_st_obj.create(
                            cr, uid, st_vals, context=context)
                        bk_st_ids.append(bk_st_id)

                # pass 2 : prepare bank statement line values and merge
                ...
                for x in lines:
                        ...
                        if line['type'] == 'information':
                            if line['struct_comm_type'] in parse_comms_info:
                                line['name'], line['communication'] = \
                                    parse_comm_info(self, cr, uid, line,
                                                    comm_type_table, context)
                            elif line['struct_comm_type'] in parse_comms_move:
                                line['name'], line['communication'] = \
                                    parse_comm_move(self, cr, uid, line,
                                                    comm_type_table, context)
                        elif line['struct_comm_type'] in parse_comms_move:
                                line['name'], line['communication'] = \
                                    parse_comm_move(self, cr, uid, line,
                    ...
                    # handling transactional records
                    # line['type'] in ['globalisation', 'general', 'supplier', 'customer'] 

                    if line['type'] in ['globalisation', 'general', 'supplier', 'customer']:
                        ...
                        if line['type'] == 'globalisation':
                            ...
                            coda_st_line_id = statement['type'] == 'info' and coda_st_line_obj.create(cr, uid, {
                            ...
                            if statement['type'] == 'normal':
                                ...
                                # To do : move reconcile handling to separate method
                                ...
                                if line['amount'] != 0.0:
                                    ...
                                    st_line_vals_in = {
                                           ...
                                           'note': line_note,
                                           # values infra must be removed before create 
                                           'action': line.get('action'),
                                           'reconcile': line.get('reconcile'),
                                            }

                                    st_line_vals = self._st_line_hook(cr, uid, statement['coda_bank_params'], line, st_line_vals_in, context=context)
                                    statement_lines += st_line_vals
                                    if not st_line_vals:
                    ...
                    # handling non-transactional records : line['type'] in ['information', 'communication']
                        ...
                        if statement['type'] == 'info':
                            st_line_id = coda_st_line_obj.create(cr, uid, {
                                'sequence': line['sequence'],
                                'ref': line['ref'],
                                'name': line['name'].strip() or '/',
                                'type' : 'information',
                            ...
                            # update statement line values generated from the 2.x move record
                            for st_line_vals in statement_lines:
                                if line['ref_move'] == st_line_vals['ref'][:4]:
                                    st_line_vals['note'] += '\n' + line['communication']
                ...
                # creation of bank statement lines, account vouchers, account moves
                if statement['type'] == 'normal':
                    st_line_seq = 0  # resequence since _st_line_hook hook may add extra lines
                    st_balance_end = round(statement['balance_start'], self.digits)
                    for st_line in statement_lines:
                        action = st_line['action']
                        del st_line['action']
                        reconcile = st_line['reconcile']
                        del st_line['reconcile']
                        tax_code_id = st_line['tax_code_id']
                        del st_line['tax_code_id']
                        st_line_seq += 1
                        st_balance_end += round(st_line['amount'], self.digits)
                        st_line.update({'sequence': st_line_seq})
                        bank_st_line_id = bank_st_line_obj.create(cr, uid, st_line, context=context)
                        if st_line['voucher_id']:
                            voucher = voucher_obj.browse(cr, uid, st_line['voucher_id'], context=context)
                            move_line = move_line_obj.browse(cr, uid, reconcile, context=context)
                            if st_line['amount'] == (st_line['amount'] > 0 and move_line.debit or -move_line.credit):
                                try:
                                    wf_service.trg_validate(uid, 'account.voucher', st_line['voucher_id'], 'proforma_voucher', cr)
                                except (orm.except_orm, osv.except_osv), e:
                                    statement['coda_parsing_note'] += _("\nReconciliation Error in Bank Statement '%s' line '%s':\n    %s") \
                                        %(statement['name'], st_line['ref'], e[1])
                            if voucher.move_id:
                            ...
                            if tax_code_id:
                                ctx_move_create['tax_code_id'] = tax_code_id
                    ...
                    if round(st_balance_end - statement['balance_end_real'], self.digits):
                        err_string += _('\nIncorrect ending Balance in CODA Statement %s for Bank Account %s!')  \
                            % (statement['coda_seq_number'], (statement['acc_number'] + ' (' + statement['currency'] + ') - ' + statement['description']))
                        statement['coda_parsing_note'] += '\n' + err_string

                    bank_st_obj.button_dummy(cr, uid, [bk_st_id], context=context)      # calculate balance_end
                    journal_name = journal.name

                else:  # type 'info'
                    journal_name = _('None')

                coda_import_note = coda_import_note + \
                    _('\n\nBank Journal: %s'
                    '\nCODA Version: %s'
                ...
                if statement['type'] == 'normal' and statement['coda_note']:
                    bank_st_obj.write(cr, uid, [bk_st_id], {'coda_note': statement['coda_note']})
        ...
        coda_note_footer = '\n\n' + _('Number of statements processed') + ' : ' + str(len(coda_statements))
        err_log = err_log + '\n' + _('Number of errors') + ' : ' + str(nb_err) + '\n'

        if not nb_err:
            if context.get('coda_id'):
                old_note = coda_obj.read(
                    cr, uid, coda_id, ['note'], context=context)['note'] or ''
                old_note += '\n\n'
            else:
                old_note = ''
            note = coda_note_header + coda_import_note + coda_note_footer
            coda_obj.write(
                cr, uid, [coda_id], 
        ...
        model_data_ids = mod_obj.search(
            cr, uid, [
            ('model', '=', 'ir.ui.view'),
            ('name', '=', 'account_coda_import_result_view')],
            context=context)
    ...
    def _match_payment_reference(self, cr, uid, st_line, coda_parsing_note, coda_statement, context=None):
        """ placeholder for ISO 20022 Payment Order matching, cf. module l10n_be_coda_pain """
        return st_line, coda_parsing_note, {}
    ...
    def _coda_statement_init(self, cr, uid, coda_statement, 
                             coda_import_note, context=None):
        """ 
        ...
        Returns:
            ...
            coda_import_note: e.g. add extra info to the CODA Note
        """
        return coda_statement, coda_import_note

    def _statement_hook(self, cr, uid, coda_bank_params,
                        st_vals, context=None):
        """ 
        Use this method to take customer specific actions based upon
    ...
    def _st_line_hook(self, cr, uid, coda_bank_params,
                      line, st_line_vals, context=None):
        """ 
        Use this method to adapt the statement line created by the
        CODA parsing to customer specific needs.

        Remark:
        Please ensure that at least one of the returned lines
        has retained the original 'move_ref'.
        """
        return [st_line_vals]
    ...
    def action_open_coda_statements(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        module, xml_id = 'l10n_be_coda_advanced', 'action_coda_bank_statements'
