...
class account_coda(orm.Model):
    ...
    _columns = {
        ...
        'coda_statement_ids': fields.one2many('coda.bank.statement', 'coda_id','Generated CODA Bank Statements', readonly=True),
        'bank_statement_ids': fields.one2many('account.bank.statement', 'coda_id', 'Generated Bank Statements', readonly=True),
    ...
    def process(self, cr, uid, ids, context=None):
        ...
        wiz_obj = self.pool.get('account.coda.import')
        assert len(ids) == 1, \
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for coda in self.browse(cr, uid, ids, context=context):
            bank_st_obj.unlink(cr, uid, [x.id for x in coda.bank_statement_ids], context=context)
...
class account_coda_trans_type(orm.Model):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        if not context.get('mapping_rule'):
            return super(account_coda_trans_type, self).name_get(cr, uid, ids, context)
...
class coda_account_mapping_rule(orm.Model):
    ...
    def rule_get(self, cr, uid, coda_bank_account_id,
        ...
        select += self._rule_select_extra(
            cr, uid, coda_bank_account_id, context=context) + 'action '
        select += \
        ...
        result_fields = [
            'account_id', 'tax_code_id', 'analytic_account_id', 'action']
        result_fields += self._rule_result_extra(
            cr, uid, coda_bank_account_id, context=context)
        res = {}
        for rule in rules:
            if eval(condition):
