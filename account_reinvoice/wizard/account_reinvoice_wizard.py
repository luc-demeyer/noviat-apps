        ...
        if self.uninvoiced:
            reinvoice_lines = self.env['account.reinvoice.line'].search(
                [('invoice_line_out_id', '=', False),
                 ('company_id', '=', self.company_id.id)])
        invoices = self._create_invoices(reinvoice_lines)
        note = _("Number of Selected Journal Items: %s") % len(amls) + "\n"
        out_invoice_ids = [
            inv.id for inv in
            filter(lambda x: x.type == 'out_invoice', invoices)]
        out_refund_ids = [
            inv.id for inv in
            filter(lambda x: x.type == 'out_refund', invoices)]
        note += _(
            "Number of Outgoing Invoices: %s") % len(out_invoice_ids) + "\n"
        note += _(
            "Number of Outgoing Refunds: %s") % len(out_refund_ids) + "\n"
        if self._note:
            note += "\n\n" + _("Remarks") + ":\n"
            note += self._note
        self.note = note
