...
from openerp.addons.base_iban.base_iban \
    import _ref_iban, _format_iban
import logging
...
class res_partner_bank(orm.Model):
    ...
    def name_search(self, cr, user, name, args=None, operator='ilike',
        ...
        if name:
            ids = self.search(
                cr, user, [('acc_number', operator, name)] + args,
                limit=limit)
        else:
            ids = self.search(cr, user, args, context=context, limit=limit)
        return self.name_get(cr, user, ids, context=context)

    def _rpb_select(self, acc_number, context=None):
        select = """
        SELECT id FROM
         (SELECT id, state, acc_number, %s AS len_acc_number
          FROM res_partner_bank) b
         WHERE
           /* Belgium: BBAN is always 12 chars and subset of IBAN */
           state='iban' AND UPPER(SUBSTRING(acc_number FOR 2)) = 'BE'
           AND len_acc_number = 12
           AND UPPER(REPLACE(REPLACE(acc_number, ' ', ''), '-', ''))
               LIKE '%%'|| '%s' ||'%%'
           /* other countries */
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None,
        ...
        The code below could be simplified if account numbers
        were stored in the database in 'normalised' format
        ...
        for arg in args:
            if arg[0] == 'acc_number' and arg[2]:
                ...
                if arg[1] in ['=', '=like', '=ilike']:
                    cr.execute(self._rpb_select(acc_number))
                    res = cr.fetchall()
                    if res:
        ...
        return super(res_partner_bank, self).search(
            cr, uid, new_args, offset, limit, order,
