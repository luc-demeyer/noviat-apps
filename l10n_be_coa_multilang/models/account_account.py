...
class AccountAccount(models.Model):
    ...
    def _compute_company_country_code(self):
        for account in self:
            account.company_country_code = (
                account.company_id.country_id and
                account.company_id.country_id.code.upper())
    ...
    def _onchange_code(self):
        ...
        if self.code and \
            ...
            for chart_entry in self.be_report_chart_ids:
                if (chart_entry in be_chart_entries and
                        chart_entry != entry.report_chart_id):
    ...
    def create(self, vals):
        ...
        if company.country_id.code in self._get_be_scheme_countries():
            entry, entries = self._get_be_report_scheme_entry(
                vals.get('code', ''))
            if entry:
                if entry.report_chart_id:
                    vals['be_report_chart_ids'] = [
                        (4, entry.report_chart_id.id)]
