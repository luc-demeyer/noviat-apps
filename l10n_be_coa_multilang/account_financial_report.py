...
class be_legal_financial_reportscheme(orm.Model):
    ...
    def _get_account_ids(self, cr, uid, ids, field_name, arg, context=None):
        result = {}
        for record in self.browse(cr, uid, ids, context=context):
            result[record.id] = [x.id for x in record.report_id.account_ids]
        return result    
    
    _columns = {
        'account_group': fields.char('Group', size=4, required=True, help='General Account Starting Digits'),
        'report_id': fields.many2one('account.financial.report', 'Report Entry', required=True, ondelete='cascade'),
        #'account_ids': fields.related('report_id','account_ids', type='many2many', relation='account.account', string='Accounts', readonly=True),
        # function field in stead of related since m2m related doesn't work correctly in multi-company environments
        'account_ids': fields.function(_get_account_ids, type='many2many', relation="account.account", string='Accounts'),
