...
class be_legal_financial_reportscheme(orm.Model):
    ...
    _columns = {
        ...
        'account_ids': fields.related('report_id', 'account_ids', type='one2many',
            relation='account.account', string='Accounts', readonly=True),
    }
    _sql_constraints = [
...
class account_financial_report(orm.Model):
    ...
    def _get_children_by_order(self, cr, uid, ids, context=None):
        ...
        if context.get('get_children_by_sequence'):
            res = self.search(cr, uid, [('id', 'child_of', ids[0]), ('invisible', '=', 0)],
                order='sequence ASC', context=context)
        else:
            res = super(account_financial_report, self)._get_children_by_order(cr, uid, ids, context)
        return res
...
class account_account(orm.Model):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        scheme_table = scheme_obj.read(cr, uid, scheme_obj.search(cr, uid, []),
            ['account_group', 'report_id'], context=context)
            ...
            if be_report_entries:
                if len(be_report_entries) > 1:
                    raise orm.except_orm(_('Configuration Error !'),
                        _('Configuration Error in the Belgian Legal Financial Report Scheme.'))
                    ...
                    for fin_report in account.financial_report_ids:
                        if fin_report.id in be_report_ids:
                            if acc_type not in ['view', 'consolidation'] and fin_report.id == be_report_id:
                                ...
                                updated = True
                    if be_report_id and (acc_type not in ['view', 'consolidation'] or \
                        (acc_type == 'view' and centralized)) and not updated:
...
class l10n_be_update_be_reportscheme(orm.TransientModel):
    ...
    def update_be_reportscheme(self, cr, uid, ids=None, context=None):
        ...
        account_ids = acc_obj.search(cr, uid,
            ['|', ('type', '!=', 'view'), '&', ('type', '=', 'view'), ('centralized', '=', True)], context=upd_ctx)
        ...
        for ca in centralized_accounts:
            ...
            if config_errors:
                ...
                note += _("A centralized account that is a child of a parent centralized account is not supported !\n"
                        ...
                        % (', '.join([x.code + ' - ' + x.name + ' (' + x.company_id.name + ')'  for x in config_errors]))
        ...
        if note:
            self.write(cr, uid, ids[0], {'note': note})
            model, res_id = mod_obj.get_object_reference(cr, uid,
