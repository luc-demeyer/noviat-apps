...
class l10n_be_vat_declaration(orm.TransientModel):
    ...
    def _get_datas(self, cr, uid, ids, context=None):
        ...
        if not address_id.email:
            raise (_('Data Insufficient!'),_('No email address associated with the company.'))
        if not address_id.phone:
            raise (_('Data Insufficient!'),_('No phone associated with the company.'))
        xml_dict = {
                        'issued_by': issued_by,
                        'vat_no': vat_no,
                        'only_vat': vat_no[2:],
        ...
        for item in tax_info:
            if tax_info['91'][1] and ending_month != 12:
                ...
                raise (_('Incorrect Data!'),_('Tax Case 91 is only allowed for the declaration of December.'))
...
<ns2:VATConsignment xmlns="http://www.minfin.fgov.be/InputCommon" xmlns:ns2="http://www.minfin.fgov.be/VATConsignment" VATDeclarationsNbr="1">
    ...
    """ % (xml_data)
        ...
        self.write(cr, uid, ids, {
            ...
            'name': 'vat_declaration_%s_%s.xml' %(year, starting_month != ending_month and ('Q' + quarter) or starting_month.ljust(2,'0')),
            }, context=context)
