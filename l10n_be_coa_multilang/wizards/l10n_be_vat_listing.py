...
class l10nBeVatListing(models.TransientModel):
    ...
    def print_declaration(self):
        module = __name__.split('addons.')[1].split('.')[0]
        return self.env.ref(
            '%s.action_report_l10nbevatlisting' % module).report_action(self)
    ...
    def _node_ClientListing(self, parent, ns_map, ref):
        ...
        TurnOverSum = sum(
            [x['base_amount'] for x in list(client_list.values())])
        VATAmountSum = sum(
            [x['vat_amount'] for x in list(client_list.values())])
...
class l10nBeVatListingClient(models.TransientModel):
    ...
    _description = 'Annual Listing line'
