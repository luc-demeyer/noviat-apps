...
class payment_order(orm.Model):
    ...
    def _total_line_amount(self, cursor, user, ids, name, args, context=None):
        ...
        for order in self.browse(cursor, user, ids, context=context):
            if order.line_ids:
                res[order.id] = reduce(lambda x, y: x + y.amount_currency, order.line_ids, 0.0)
            else:
                res[order.id] = 0.0
        return res

    _columns = {
        'total_line_amount': fields.function(_total_line_amount, string="Sum amounts", type='float',
            help="Total of all individual amounts included in the message, irrespective of currencies"),
    }
