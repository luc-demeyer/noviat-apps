...
class account_invoice_line(orm.Model):
    ...
    def _inv_name_default(self, cr, uid, context=None):
        ...
        if context is None:
            context = {}
        if context and 'inv_name' in context:
    ...
    def _inv_account_default(self, cr, uid, context=None):
        ...
        if context is None:
            context = {}
        if context and 'inv_type' in context:
            if context.get('inv_partner_id'):
                partner = partner_obj.browse(
                    cr, uid, context['inv_partner_id'])
                # replace by the partner for which the accounting entries
                # will be created
        ...
        if context and 'inv_type' in context:
            if context.get('inv_partner_id'):
                ...
                if account_id:
                    ...
                    fpos = fposition_id \
                        and fpos_obj.browse(cr, uid, fposition_id) or False
